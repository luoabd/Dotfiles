...
local lock_animation_icon = wibox.widget({
	...
	forced_width = dpi(80),
	font = "Material Icons " .. "Outlined 40",
	align = "center",
	valign = "center",
...
lock_screen_box.bg = "#00000000" ---transparent
lock_screen_box.fg = "#edeff0" ---white
...
awful.screen.connect_for_each_screen(function(s)
	if s == screen.primary then
		...
		s.mylockscreen = helpers.ui.screen_mask(
			s,
			beautiful.lock_screen_bg or beautiful.exit_screen_bg or "#0c0e0f" ---black
		)
	end
end)
...
local function activate_word(w)
	for i, m in pairs(char_map[w]) do
		local text = m.text
		m.markup = helpers.ui.colorize_text(text, "#edeff0")
	end
end
